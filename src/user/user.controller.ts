...
export class UserController {
    ...
    @Post('auth/token')
    async auth(@Body() authData: AuthDto, @Req() req: UserRequest): Promise<User> {
        return await this.userService.auth(req);
    }

    @UseGuards(JwtAccessGuard)
    @Post('me/fcm/topics')
    async createSubscriptionToFcmTopics(@Req() req: UserRequest, @Body() tokenData: FcmTopicDto) {
        return await this.userService.createSubscriptionToFcmTopics(req, tokenData);
    }

    @UseGuards(JwtAccessGuard)
    @Delete('me/fcm/topics')
    async deleteSubscriptionFromFcmTopics(@Req() req: UserRequest, @Body() tokenData: FcmTopicDto) {
        return await this.userService.deleteSubscriptionFromFcmTopics(req, tokenData);
    }
}
