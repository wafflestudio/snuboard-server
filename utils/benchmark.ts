...
async function search(keyword: string, departments: number[], searchType: string) {
    ...
    for (let i = 0; i < departments.length; i++) {
        ...
        if (searchType == 'LIKE') {
            ...
            noticeQb.andWhere('match(contentText, title) against (:keyword IN BOOLEAN MODE)', {
                keyword: `+${keyword}*`,
            });
        }
...
async function runBenchmark() {
    ...
    for (let i = 0; i < keywords.length; i++) {
        ...
        const sumLike = resultLike.map((r) => r.elapsedTime).reduce((a, b) => a + b);
        const sumFTS = resultFTS.map((r) => r.elapsedTime).reduce((a, b) => a + b);
        console.log(``);
        console.log(`keyword: ${keywords[i]}`);
        console.log(`elapsed time - like: ${(sumLike / resultLike.length).toFixed(2)}ms`);
        console.log(`num of notices - like: ${resultLike[0].noticeNum}`);

        //console.log(resultLike);
        console.log(`elapsed time - FTS: ${(sumFTS / resultFTS.length).toFixed(2)}ms`);
