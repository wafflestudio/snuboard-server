...
export class NoticeService {
    ...
    async getNoticeInDepartment(
        ...
        if (department === undefined)
            throw new BadRequestException(`There is no department with the id ${departmentId}`);
        ...
        if (queryTags.length !== tags.length) {
            throw new BadRequestException(
                `There is no tag with the name ${queryTags.filter((t) => !tagNames.includes(t))}`,
            );
        }

        const noticeQb: SelectQueryBuilder<Notice> = Notice.createQueryBuilder('notice');
        if (tags.length === 0) {
            noticeQb.andWhere('notice.departmentId = :departmentId').setParameter('departmentId', departmentId);
        }
        if (query.pinned) {
        ...
        if (this.isSearchQuery(query)) {
            return await this.searchNotice(noticeQb, user, query, tagIds, departmentId);
        }
    ...
    async getFollowedNotice(req: UserRequest, query: NoticePaginationDto): Promise<NoticesResponseDto> {
        ...
        this.appendTagQb(noticeQb, tagIds);
        return await this.makeResponse(noticeQb, user, query.limit, query.cursor, 'noticeTag');
    }

    async getScrappedNotice(req: UserRequest, query: NoticePaginationDto): Promise<NoticesResponseDto> {
        const { user } = await this.validateQuery({
            query,
            user: req.user,
        });
        ...
        const noticeIds: number[] = userNotices.map((userNotice) => userNotice.notice.id);
        const noticeQb: SelectQueryBuilder<Notice> = Notice.createQueryBuilder('notice').whereInIds(noticeIds);
        return await this.makeResponse(noticeQb, user, query.limit, query.cursor, 'notice');
    }
    ...
    async searchNotice(
        ...
        return await this.makeResponse(noticeQb, user, query.limit, query.cursor, orderByCol);
    }
    ...
    async makeResponse(
        ...
        const createdAtCol = orderByCol == 'notice' ? 'notice.createdAt' : 'noticeTag.noticeCreatedAt';

        const noticeIdCol = orderByCol == 'notice' ? 'notice.id' : 'noticeTag.noticeId';
        ...
        if (notices.length > limit) {
            ...
            noticesResponse.next_cursor = `${lastNotice.createdAt.getTime()}-${lastNotice.id}`;
        } else {
    ...
    async validateQuery(preQuery: PreQuery): Promise<UserDepartment> {
        ...
        if (
            ...
            (cursorSplit.length !== 2 || Number.isNaN(+cursorSplit[0]) || Number.isNaN(+cursorSplit[1]))
        ) {
            throw new BadRequestException('invalid cursor format');
        }
        if (this.isSearchQuery(query)) {
            const minLength = Math.min(...this.splitParam(query.keywords, ' ').map((keyword) => keyword.length));
            if (minLength < 2) {
                throw new BadRequestException('각 단어의 길이는 2 이상이어야 합니다');
            }
        }
        if (departmentId) {
            const department: Department | null = await Department.findOne({ where: { id: departmentId } });
            if (!department) {
                throw new BadRequestException(`There is no department with the id ${departmentId}`);
            }
    ...
    appendKeywordQb(noticeQb: SelectQueryBuilder<Notice>, keywords: string[], departments: string[]): void {
        const keywordParam = keywords
            .map((keyword) => `+${keyword.replace(/[-*+~()<>"]+/g, '')}*`)
            .reduce((a, b) => `${a} ${b}`, '');
        const departmentParam = `+(${departments.reduce((acc, dept) => `${acc} ${dept}`, '')})`;

        noticeQb
            .andWhere(`match (contentText, title, departmentCode) against (:keywordParam IN BOOLEAN MODE)`)
    ...
    async getTagsFromName(tagNames: string[], department: Department): Promise<Tag[]> {
        return (
            (await Tag.find({
                where: {
                    name: In(tagNames),
    ...
    isSearchQuery(query: NoticePaginationDto): query is SearchNoticeInDeptDto | SearchFollowedNoticeDto {
        return (<SearchNoticeInDeptDto | SearchFollowedNoticeDto>query).keywords !== undefined;
    }
    ...
    async validateNotice(req: UserRequest, id: number): Promise<PreNotice> {
        ...
        const notice: Notice | null = await Notice.findOne({
            where: { id },
    ...
    async deleteScrap(req: UserRequest, id: number): Promise<Notice> {
        ...
        if (!userNotice || !userNotice.isScrapped) {
            throw new HttpException('관심목록에 없는 공지사항입니다.', HttpStatus.NO_CONTENT);
        }
